...
def _apply_updates(existing_item: Dict[str, Any], updates: Dict[str, Any]) -> Dict[str, Any]:  
    """
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if not auth_token or not validate_token(auth_token):  
        return _error_response(  
            403, "Authentication failed", "AUTHENTICATION_FAILED"  
        )  
    ...
    if table is None:  
        ...
        return _error_response(  
            500, "DynamoDB table not available", "DDB_NOT_AVAILABLE"  
        )  
    ...
    try:  
        ...
        logger.error(f"Failed to load artifact {artifact_id} from DynamoDB: {e}", exc_info=True)  
        return _error_response(  
            500, "Failed to load artifact", "DDB_GET_FAILED"  
        )  

    item = response.get("Item")  
    if not item:  
        return _error_response(  
            404, f"Artifact {artifact_id} not found", "NOT_FOUND"  
        )  
    ...
    try:  
        ...
        logger.error(f"Failed to update artifact {artifact_id} in DynamoDB: {e}", exc_info=True)  
        return _error_response(  
            500, "Failed to update artifact", "DDB_PUT_FAILED"  
        )  
