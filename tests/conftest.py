...
def test_compute_scores_parallel(mock_metrics, model_artifact):
    """Verify that _compute_scores runs all metrics in parallel and populates scores."""
    # Patch METRICS with mock metrics
    with patch("src.artifacts.model_artifact.METRICS", mock_metrics):
        model_artifact._compute_scores()

    # All metrics should be present in scores
