...
def test_treescore_ancestor_chain(metric):
	grandparent = DummyModelArtifact(artifact_id="g1", parent_model_id=None, scores={"net_score": 0.6})
	...
	with patch('src.metrics.treescore_metric.load_artifact_metadata', side_effect=load_artifact_metadata_side_effect):
		result = metric.score(model)
