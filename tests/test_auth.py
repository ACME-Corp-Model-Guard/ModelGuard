...
def test_authenticate_user_success(auth_module, mock_boto_clients):
    ...
    mock_boto_clients["cognito"].return_value.initiate_auth.return_value = fake_auth_result
...
def test_authenticate_user_cognito_failure(auth_module, mock_boto_clients):
    mock_boto_clients["cognito"].return_value.initiate_auth.side_effect = Exception("boom")
...
def test_verify_token_signature_fail(auth_module, mocker):
    ...
    mocker.patch("src.auth.jwt.get_unverified_claims", return_value={"exp": time.time() + 1000})
...
def test_verify_token_expired_jwt(auth_module, mocker):
    ...
    table.put_item(Item={"token": "abc", "username": "x", "issued_at": int(time.time()), "uses": 0})

    mocker.patch("src.auth.jwt.get_unverified_header", return_value={"kid": "testkid"})
    mocker.patch("src.auth.jwt.get_unverified_claims", return_value={"exp": time.time() - 5})
