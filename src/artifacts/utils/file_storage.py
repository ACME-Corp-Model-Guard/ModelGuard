...
def _download_from_huggingface(source_url: str, artifact_id: str) -> str:
    ...
    try:
        ...
        raise FileDownloadError("huggingface_hub library required for HuggingFace downloads. Install with: pip install huggingface_hub")
...
def upload_artifact_to_s3(artifact_id: str, s3_key: str, source_url: str) -> None:
    ...
    try:
        ...
        logger.error(f"Failed to upload artifact {artifact_id} to S3: {e}", exc_info=True)
        raise
    except Exception as e:
...
def download_artifact_from_s3(artifact_id: str, s3_key: str, local_path: str) -> None:
    ...
    logger.debug(f"Downloading artifact {artifact_id} from s3://{bucket_name}/{s3_key} to {local_path}")
    ...
    try:
        ...
        logger.error(f"Failed to download artifact {artifact_id} from S3: {e}", exc_info=True)
        raise
